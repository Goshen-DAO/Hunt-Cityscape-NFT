...
export function Inventory({ nft }: Props) {
  ...
  useEffect(() => {
    if (toolContractError) {
      console.error("Error connecting to Building contract:", toolContractError);
    }
  }, [toolContractError]);
  ...
  const handleEquipSuccess = () => {
    toast({
      ...
      description: "You have successfully equipped a Building.",
      status: "success",
      duration: 5000,
      isClosable: true,
    });
  ...
  const handleEquipError = (error: any) => {
    ...
    toast({
      title: "Equip Error",
      description: "Something went wrong while equipping a building.",
      status: "error",
      duration: 5000,
      isClosable: true,
    });
